...
fictional drug, `RS2023`. As part of the NIF package, a synthetic SDTM data set
for this study (study `RS2023-0022`) is included as `examplinib_poc`:
...
For this data set, the LB domain contains baseline creatinine data which can be
used to calculate the baseline creatinine clearance as an estimate for GFR
(eGFR). In the last step, we clean up the NIF data set, keeping only the fields
...
By the way, this NIF data set is already included in the package under the name
'examplinib_poc_nif'. It is equivalent to the above created data set.

# EXPLORATION

## Demographics

The number of subjects, the sex distributions and other baseline
demographic data can be inspected with `summary()`. As we have added baseline
eGFR to the data set, the output also summarizes the number of patients with 
...
For a visual overview of the dataset `plot()` can be applied to the summary:
