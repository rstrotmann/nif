...
This vignette describes how to import NIF data sets created not with make_nif().
...
* `ANALYTE` (character), the name of the analyte/the administered drug. This field
is derived from the `PCTESTCD` variable of the SDTM domain 'PC'. Please see the
`vignette("nif-vignette")` and the documentation to `add_treatment_mapping()`
for details on how the analyte name can be linked to the respective drug name
(i.e., `EXTRT` in the SDTM domain 'EX'). The `ANALYTE` name is used for both,
observations and administrations of the compound.
