...
vignette: >
  %\VignetteIndexEntry{Importing NIF data sets}
...
csv_content <- utils::capture.output(
...
examplinib_sad_min_nif |>
  filter(ID < 3) |> 
  filter(TIME < 8) |> 
  write_nonmem(sep = ",", filename = csv_file_name)
...
cat(paste(csv_content, collapse = "\n"))
```
...
The resulting object is a regular `nif` object. It can be summarized or plotted
or otherwise explored (see `vignette("nif-tutorial")` for more information):
