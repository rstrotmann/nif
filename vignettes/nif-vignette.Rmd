...
The package is intended to facilitate the creation of analysis data sets ('NIF
...
object, and how to explore clinical data based on the SDTM level.
...
fully cleaned are analyzed. In addition, study-specific
...
they specify the sequence of fasted and fed administrations (see suggestion 1).
...
and `ACTARMCD` in the output of `suggest()` above). In many studies, relevant
fields for consideration are `EPOCH` and `ACTARMCD`:
...
(for more detail, refer to `?plot.nif()`).
