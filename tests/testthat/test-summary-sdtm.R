...
test_that("summary.sdtm handles objects with complete mapping data", {
  # Create SDTM object with complete mapping data
  test_data <- list(
    domains = list(
    ...
    analyte_mapping = data.frame(
      ...
      PCTESTCD = c("DRUGA", "DRUGB"),
      ANALYTE = c("ANALYTE_A", "ANALYTE_B")
    ),
    metabolite_mapping = data.frame(
      PCTESTCD_parent = c("DRUGA", "DRUGB"),
      PCTESTCD_metab = c("DRUGA_METAB", "DRUGB_METAB")
    ),
    time_mapping = data.frame(
      PCTPT = c("PRE", "1HR", "2HR"),
      NTIME = c(0, 1, 2)
  ...
  # Check that mappings are correctly included
  expect_s3_class(result, "summary_sdtm")
  expect_equal(nrow(result$analyte_mapping), 2)
  expect_equal(nrow(result$metabolite_mapping), 2)
  expect_equal(nrow(result$time_mapping), 3)

  # Check print method includes mappings
  expect_output(print(result), "Treatment-to-analyte mappings")
