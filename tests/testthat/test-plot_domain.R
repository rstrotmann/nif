# Test file for plot.sdtm_domain() function

test_that("plot.sdtm_domain validates input parameters correctly", {
...
test_that("plot.sdtm_domain works with DM domain", {
  ...
  # Test basic DM plot
  p <- plot.sdtm_domain(test_dm)
  expect_s3_class(p, "ggplot")


  # Test DM plot with points = FALSE
  p <- plot.sdtm_domain(test_dm, points = FALSE)
  expect_s3_class(p, "ggplot")

  # Test DM plot with color
  p <- plot.sdtm_domain(test_dm, color = "SEX")
  expect_s3_class(p, "ggplot")

  # Test DM plot with color and points = FALSE
  p <- plot.sdtm_domain(test_dm, color = "SEX", points = FALSE)
  expect_s3_class(p, "ggplot")

  # Test DM plot with legend = FALSE
  p <- plot.sdtm_domain(test_dm, legend = FALSE)
  expect_s3_class(p, "ggplot")
})


test_that("plot.sdtm_domain handles DM domain missing fields", {
  ...
  class(test_dm_missing_rfstdtc) <- c("sdtm_domain", "data.frame")

  expect_error(
    plot.sdtm_domain(test_dm_missing_rfstdtc),
  ...
  class(test_dm_missing_rfendtc) <- c("sdtm_domain", "data.frame")

  expect_error(
    plot.sdtm_domain(test_dm_missing_rfendtc),
  ...
  class(test_dm_missing_usubjid) <- c("sdtm_domain", "data.frame")

  expect_error(
    plot.sdtm_domain(test_dm_missing_usubjid),
  ...
  class(test_dm_missing_multiple) <- c("sdtm_domain", "data.frame")

  expect_error(
    plot.sdtm_domain(test_dm_missing_multiple),
    "missing fields:"
  )
})


test_that("plot.sdtm_domain works with EX domain", {
  ...
  class(test_ex) <- c("sdtm_domain", "data.frame")

  # Test basic EX plot
  p <- plot.sdtm_domain(test_ex)
  expect_s3_class(p, "ggplot")

  # Test EX plot with points = FALSE
  p <- plot.sdtm_domain(test_ex, points = FALSE)
  expect_s3_class(p, "ggplot")

  # Test EX plot with legend = FALSE
  p <- plot.sdtm_domain(test_ex, legend = FALSE)
  expect_s3_class(p, "ggplot")
})


test_that("plot.sdtm_domain handles EX domain missing fields", {
  ...
  class(test_ex_missing_exstdtc) <- c("sdtm_domain", "data.frame")

  expect_error(
    plot.sdtm_domain(test_ex_missing_exstdtc),
  ...
  class(test_ex_missing_exendtc) <- c("sdtm_domain", "data.frame")

  expect_error(
    plot.sdtm_domain(test_ex_missing_exendtc),
    "missing field: EXENDTC"
  )
})


test_that("plot.sdtm_domain works with generic domains (LB)", {
  ...
  class(test_lb) <- c("sdtm_domain", "data.frame")

  # Test basic LB plot
  p <- plot.sdtm_domain(test_lb)
  expect_s3_class(p, "ggplot")


  # Test LB plot with lines = TRUE
  p <- plot.sdtm_domain(test_lb, lines = TRUE)
  expect_s3_class(p, "ggplot")

  # Test LB plot with points = FALSE, lines = TRUE
  p <- plot.sdtm_domain(test_lb, points = FALSE, lines = TRUE)
  expect_s3_class(p, "ggplot")

  # Test LB plot with testcd filter
  p <- plot.sdtm_domain(test_lb, testcd = "ALT")
  expect_s3_class(p, "ggplot")

  # Test LB plot with legend = FALSE
  p <- plot.sdtm_domain(test_lb, legend = FALSE)
  expect_s3_class(p, "ggplot")

  # Test LB plot with color
  p <- plot.sdtm_domain(test_lb, color = "SEX")
  expect_s3_class(p, "ggplot")
})


test_that("plot.sdtm_domain works with generic domains (PC)", {
  ...
  # Test basic PC plot
  p <- plot.sdtm_domain(test_pc)
  expect_s3_class(p, "ggplot")


  # Test PC plot with lines = TRUE
  p <- plot.sdtm_domain(test_pc, lines = TRUE)
  expect_s3_class(p, "ggplot")

  # Test PC plot with testcd filter
  p <- plot.sdtm_domain(test_pc, testcd = "RS2023")
  expect_s3_class(p, "ggplot")
})


test_that("plot.sdtm_domain works with generic domains using DY field", {
  ...
  class(test_vs) <- c("sdtm_domain", "data.frame")

  # Test VS plot (should use VSDY as time field)
  p <- plot.sdtm_domain(test_vs)
  expect_s3_class(p, "ggplot")

})


test_that("plot.sdtm_domain handles domains without required fields gracefully", {
  ...
  class(test_minimal) <- c("sdtm_domain", "data.frame")

  expect_error(
    result <- plot.sdtm_domain(test_minimal),
    "Missing domain fields: XXTESTCD, XXDTC and XXSTRESN"
  )
})


test_that("plot.sdtm_domain filters NA values correctly", {
  ...
  class(test_lb_na) <- c("sdtm_domain", "data.frame")

  # Should filter out NA values and still produce a plot
  p <- plot.sdtm_domain(test_lb_na)
  expect_s3_class(p, "ggplot")
})


test_that("plot.sdtm_domain works with real examplinib data", {
  # Test with actual examplinib data if available
  if (exists("examplinib_sad")) {
    # Test LB domain
    lb_domain <- domain(examplinib_sad, "lb")
    p <- plot.sdtm_domain(lb_domain)
    expect_s3_class(p, "ggplot")


    # Test with testcd filter
    p <- plot.sdtm_domain(lb_domain, testcd = "ALT")
    expect_s3_class(p, "ggplot")

    # Test with different parameter combinations
    p <- plot.sdtm_domain(lb_domain, points = TRUE, lines = TRUE, legend = TRUE)
    expect_s3_class(p, "ggplot")

    p <- plot.sdtm_domain(lb_domain, points = FALSE, lines = TRUE, legend = FALSE)
    expect_s3_class(p, "ggplot")
  }
})


test_that("plot.sdtm_domain handles empty domain gracefully", {
  # Create empty DM domain
  test_empty_dm <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~RFSTDTC, ~RFENDTC
  )
  class(test_empty_dm) <- c("sdtm_domain", "data.frame")

  expect_warning(
    result <- plot.sdtm_domain(test_empty_dm),
    "Empty domain data"
  )
...
test_that("plot.sdtm_domain handles single subject/row", {
  # Create DM domain with single subject
  test_dm_single <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~RFSTDTC, ~RFENDTC,
    "SUBJ-001", "DM", "2023-01-01", "2023-01-10"
  )
  class(test_dm_single) <- c("sdtm_domain", "data.frame")

  p <- plot.sdtm_domain(test_dm_single)
  expect_s3_class(p, "ggplot")
  ...
  class(test_lb_single) <- c("sdtm_domain", "data.frame")

  p <- plot.sdtm_domain(test_lb_single)
  expect_s3_class(p, "ggplot")
})


test_that("plot.sdtm_domain handles testcd parameter with multiple values", {
  ...
  class(test_lb_multi) <- c("sdtm_domain", "data.frame")

  # Test with NULL testcd (should show all)
  p <- plot.sdtm_domain(test_lb_multi, testcd = NULL)
  expect_s3_class(p, "ggplot")

  # Test with specific testcd
  p <- plot.sdtm_domain(test_lb_multi, testcd = "ALT")
  expect_s3_class(p, "ggplot")

  # Test with non-existent testcd (should result in empty plot or no error)
  p <- plot.sdtm_domain(test_lb_multi, testcd = "NONEXISTENT")
  expect_s3_class(p, "ggplot")
})


test_that("plot.sdtm_domain applies theme and watermark correctly", {
  # Create test domain
  test_lb <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~LBDTC, ~LBSTRESN, ~LBTESTCD,
    "SUBJ-001", "LB", "2023-01-01", 10.5, "ALT"
...
test_that("plot.sdtm_domain handles legend positioning correctly", {
  # Create test domain
  test_lb <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~LBDTC, ~LBSTRESN, ~LBTESTCD,
    "SUBJ-001", "LB", "2023-01-01", 10.5, "ALT",
  ...
  p1 <- plot.sdtm_domain(test_lb, legend = TRUE)
  expect_s3_class(p1, "ggplot")

  # Test with legend = FALSE
  p2 <- plot.sdtm_domain(test_lb, legend = FALSE)
...
test_that("plot.sdtm_domain handles color parameter with different domains", {
  # Test DM with color
  test_dm <- tibble::tribble(
  ...
  p <- plot.sdtm_domain(test_dm, color = "RACE")
  expect_s3_class(p, "ggplot")
  ...
  expect_no_error(p <- plot.sdtm_domain(test_lb, color = "SEX"))
  expect_s3_class(p, "ggplot")
})


test_that("plot.sdtm_domain handles date conversion correctly", {
  ...
  class(test_lb_dates) <- c("sdtm_domain", "data.frame")

  # Should handle different date formats via lubrify_dates
  p <- plot.sdtm_domain(test_lb_dates)
  expect_s3_class(p, "ggplot")

})


test_that("plot.sdtm_domain handles generic domains with partial fields", {
  ...
  class(test_partial) <- c("sdtm_domain", "data.frame")

  # Should handle gracefully - may not create plot if required fields missing
  result <- tryCatch(
    plot.sdtm_domain(test_partial),
    error = function(e) e
  )
...
test_that("plot.sdtm_domain handles all parameter combinations", {
