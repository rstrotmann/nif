...
test_that("summary.domain handles valid domain objects correctly", {
  ...
  test_pc <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~STUDYID, ~PCTESTCD, ~PCTEST, ~PCCAT, ~PCSCAT, ~PCTPT, ~PCTPTNUM, ~PCELTM, ~EPOCH, ~VISIT,
    "SUBJ-001", "PC", "STUDY-001", "DRUGA", "Drug A", "PK", "PLASMA", "PRE", 0, "PT0H", "TREATMENT", "SCREENING",
    "SUBJ-001", "PC", "STUDY-001", "DRUGA", "Drug A", "PK", "PLASMA", "1HR", 1, "PT1H", "TREATMENT", "DAY1",
    "SUBJ-002", "PC", "STUDY-001", "DRUGA", "Drug A", "PK", "PLASMA", "2HR", 2, "PT2H", "TREATMENT", "DAY1"
  )
...
test_that("summary.domain handles domains with time points correctly", {
  ...
  test_pc <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD, ~PCTPT, ~PCTPTNUM, ~PCELTM,
    "SUBJ-001", "PC", "DRUGA", "PRE", 0, "PT0H",
    "SUBJ-001", "PC", "DRUGA", "1HR", 1, "PT1H",
    "SUBJ-001", "PC", "DRUGA", "2HR", 2, "PT2H"
  )
  class(test_pc) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with epochs correctly", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~EPOCH, ~PCTESTCD,
    "SUBJ-001", "PC", "SCREENING", "DRUGA",
    "SUBJ-001", "PC", "TREATMENT", "DRUGA",
    "SUBJ-002", "PC", "SCREENING", "DRUGA",
    "SUBJ-002", "PC", "TREATMENT", "DRUGA"
  )
...
test_that("summary.domain handles domains with categories correctly", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD, ~PCCAT, ~PCSCAT, ~VISIT,
    "SUBJ-001", "PC", "DRUGA", "PK", "PLASMA", "DAY1",
    "SUBJ-001", "PC", "DRUGA", "PK", "URINE", "DAY1",
    "SUBJ-002", "PC", "DRUGA", "PK", "PLASMA", "DAY1"
  )
  class(test_domain) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with observations correctly", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD, ~PCTEST, ~PCCAT,
    "SUBJ-001", "PC", "DRUGA", "Drug A", "PK",
    "SUBJ-001", "PC", "DRUGA", "Drug A", "PK",
    "SUBJ-002", "PC", "DRUGA", "Drug A", "PK"
  )
  class(test_domain) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with PCSPEC field", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD, ~PCTEST, ~PCSPEC,
    "SUBJ-001", "PC", "DRUGA", "Drug A", "PLASMA",
    "SUBJ-002", "PC", "DRUGA", "Drug A", "PLASMA"
  )
  class(test_domain) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with FAST field", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD, ~PCTEST, ~PCCAT, ~PCFAST,
    "SUBJ-001", "PC", "DRUGA", "Drug A", "PK", "Y",
    "SUBJ-002", "PC", "DRUGA", "Drug A", "PK", "N"
  )
  class(test_domain) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with VISIT field", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD, ~VISIT,
    "SUBJ-001", "PC", "DRUGA", "SCREENING",
    "SUBJ-001", "PC", "DRUGA", "DAY1",
    "SUBJ-002", "PC", "DRUGA", "SCREENING"
  )
  class(test_domain) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with SUBJID field", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~SUBJID, ~DOMAIN, ~PCTESTCD,
    "STUDY-001-001", "001", "PC", "DRUGA",
    "STUDY-001-002", "002", "PC", "DRUGA"
  )
  class(test_domain) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with only USUBJID field", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD,
    "STUDY-001-001", "PC", "DRUGA",
    "STUDY-001-002", "PC", "DRUGA"
  )
  class(test_domain) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with multiple DOMAIN values", {
  ...
  expect_error(
    ...
    "Multiple domain values found: PC and LB"
  )
})
...
test_that("summary.domain handles domains with missing optional fields gracefully", {
  ...
  test_domain <- tibble::tribble(
    ...
    "SUBJ-001", "PC", "STUDY-001",
    "SUBJ-002", "PC", "STUDY-001"
  )
  class(test_domain) <- c("domain", "data.frame")
...
#   test_domain <- tibble::tribble(
#     ~USUBJID, ~DOMAIN, ~STUDYID, ~PCTESTCD, ~PCTEST, ~PCCAT, ~PCSCAT, ~PCFAST, ~PCTPT, ~PCTPTNUM, ~PCELTM, ~EPOCH, ~VISIT, ~PCSPEC,
#     "SUBJ-001", "PC", "STUDY-001", "DRUGA", "Drug A", "PK", "PLASMA", "Y", "PRE", 0, "PT0H", "TREATMENT", "DAY1", "PLASMA",
#     "SUBJ-001", "PC", "STUDY-001", "DRUGA", "Drug A", "PK", "PLASMA", "Y", "1HR", 1, "PT1H", "TREATMENT", "DAY1", "PLASMA",
#     "SUBJ-002", "PC", "STUDY-001", "DRUGA", "Drug A", "PK", "PLASMA", "N", "PRE", 0, "PT0H", "TREATMENT", "DAY1", "PLASMA"
#   )
#   class(test_domain) <- c("domain", "data.frame")
#
#   # Call summary
...
test_that("summary.domain handles domains with numeric fields correctly", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD, ~PCTPTNUM, ~PCSTRESN,
    "SUBJ-001", "PC", "DRUGA", 0, 10.5,
    "SUBJ-001", "PC", "DRUGA", 1, 15.2,
    "SUBJ-002", "PC", "DRUGA", 0, 12.3
  )
  class(test_domain) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with NA values gracefully", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD, ~PCTEST, ~PCCAT,
    "SUBJ-001", "PC", "DRUGA", "Drug A", "PK",
    "SUBJ-001", "PC", "DRUGA", NA, "PK",
    "SUBJ-002", "PC", "DRUGA", "Drug A", NA
  )
  class(test_domain) <- c("domain", "data.frame")
...
test_that("summary.domain handles domains with duplicate values correctly", {
  ...
  test_domain <- tibble::tribble(
    ~USUBJID, ~DOMAIN, ~PCTESTCD, ~PCTEST, ~PCCAT, ~PCTPT,
    "SUBJ-001", "PC", "DRUGA", "Drug A", "PK", "PRE",
    "SUBJ-001", "PC", "DRUGA", "Drug A", "PK", "PRE",
    "SUBJ-002", "PC", "DRUGA", "Drug A", "PK", "PRE"
  )
  class(test_domain) <- c("domain", "data.frame")
