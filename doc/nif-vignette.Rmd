...
demonstrated below for the (fictional) sample data set `examplinib`:
...
In the case of "examplinib", there are different treatment arms defined in the
...
As no specimem was specified, `make_nif()` takes the most likely, however, the
specimen of interest can be explicitly provided (see `?make_nif()` for details).
...
By the way, note the `silent=T` modifier that omits all messages from `make_nif()`.
...
the `nif` data. Note that in many cases, the `nif` object shoudl be converted to
a data frame for this purpose (i.e., using `as.data.frame()`).
