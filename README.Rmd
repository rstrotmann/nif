...
This is a basic example to load SDTM data from SAS files and create a NIF. The path `/path/to/sdtm/data` needs to point to a location where at least the following
SAS export files from a study needs to be present:
...
In most cases, you may want to add further covariates after `make_nif()` and
before `ìndex_nif()`, consider a more complex example:
...
For further guidance see the help for individual functions and the included
vignettes (see `browseVignettes("nif")`):
